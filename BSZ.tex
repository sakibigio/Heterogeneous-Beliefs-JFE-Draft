...
Disciplined by survey data, the model matches asset-pricing, turnover, and business-cycle moments, highlighting how heterogeneous beliefs can be a direct driver of aggregate fluctuations.
	\vspace{10mm}	
	\bigskip
...
\iffalse
% For `utils/styles/qting.sty`: expose HandToMouthWorkers as a quote label even if
% the content lives in a separate file included via `\input`.
% This has no effect on the paper output, but lets referee letters use \quoting{HandToMouthWorkers}.
\q{HandToMouthWorkers}{\begingroup\def\section##1{}\def\label##1{}\IfFileExists{sections/appendix.workers.tex}{\input{sections/appendix.workers.tex}}{\input{../../../paper/sections/appendix.workers.tex}}\endgroup}
\fi
\input{sections/appendix.workers}


\newpage

\input{sections/supplementary_appendix_numerical}
